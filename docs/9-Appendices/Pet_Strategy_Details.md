...
(Where stratagies are shared across species, parameter ranges are used to seperate their behaviour.)
...
*   **Concept:** The Frog patiently waits for the price to make a significant move outside of its recent trading range. It uses Donchian Channels, which plot the highest high and lowest low over a set period. A Buy signal occurs when the price breaks *above* the recent high (upper channel), and a Sell signal occurs when the price breaks *below* the recent low (lower channel). It excels in trending markets after periods of consolidation.
*   **Parameters:**
...
# 3. Blob & Crab (RSI Mean Reversion Strategy)

*   **Strategy Type:** Oscillator / Mean Reversion
*   **Concept:** The Blob and Crab operate on the idea that prices tend to revert to their average after extreme moves. It uses the Relative Strength Index (RSI), a momentum oscillator measuring the speed and change of price movements. The Blob and Crab looks to Buy when the RSI suggests the asset is "oversold" (price dropped too quickly) and Sell when the RSI suggests it's "overbought" (price rose too quickly), anticipating a pullback.
*   **Parameters:**
    *   `bc_rsi_length`
    ...
    *   `bc_rsi_overbought_level`
        ...
        *   **Constraint:** Must be greater than `bc_rsi_oversold_level`.

---

# 4. Gorilla (MACD Trend Strategy)

*   **Strategy Type:** Trend Following (Momentum Oscillator)
*   **Concept:** The Gorilla follows momentum shifts using the Moving Average Convergence Divergence (MACD) indicator. MACD shows the relationship between two EMAs of price. A "Signal Line" (an EMA of the MACD line itself) is used for triggers. The Gorilla typically Buys when the MACD line crosses above the Signal Line (indicating increasing bullish momentum) and Sells when it crosses below (indicating increasing bearish momentum).
*   **Parameters:**
    *   `gori_macd_fast_period`
...
# 6. Ape (Blue), Ape (Red), Ape (Cult) & MrMonkee (Stochastic Oscillator Strategy)

*   **Strategy Type:** Momentum Oscillator / Reversal Indication
*   **Concept:** The Apes lie in wait for momentum to reach extreme levels, anticipating a potential reversal. They use the Stochastic Oscillator, which compares a closing price to its price range over a certain period. It generates %K (raw value) and %D (smoothed %K) lines oscillating between 0 and 100. The Apes typically Buy when the %K line crosses above the %D line in "oversold" territory (e.g., below 20) and Sells when %K crosses below %D in "overbought" territory (e.g., above 80).
*   **Parameters (5 total):**
    *   `am_stoch_k`
    ...
    *   `am_stoch_overbought`
        ...
        *   **Constraint:** Must be greater than `am_stoch_oversold`.
...
# 8. Wyatt (Bollinger Band Mean Reversion Strategy)

*   **Strategy Type:** Volatility / Mean Reversion
*   **Concept:** Wyatt plays within expected price boundaries defined by volatility. It uses Bollinger Bands, which plot bands a certain number of standard deviations above and below a central moving average. Wyatt assumes that when price touches an outer band, it's likely overextended and will revert towards the middle. It Buys when price hits the Lower Band and Sells when price hits the Upper Band, often exiting when price returns to the Middle Band.
*   **Parameters:**
    *   `wyatt_bbands_length`
...
# 9. ShadowApe, Snake, Squirrel & Raven (CCI Strategy)

*   **Strategy Type:** Momentum Oscillator / Cyclical Trading
*   **Concept:** Tries to identify cyclical turns or emerging trends by measuring price deviation from its statistical average. It uses the Commodity Channel Index (CCI), which oscillates around a zero line. High positive values suggest strong upward momentum (potentially overbought), while low negative values suggest strong downward momentum (potentially oversold). Typically Buys when CCI crosses *above* a lower threshold (signaling upward turn from oversold) and Sells when CCI crosses *below* an upper threshold (signaling downward turn from overbought).
*   **Parameters:**
    *   `sssr_cci_length`
...
# 10. TokenGator (Awesome Oscillator Strategy)

*   **Strategy Type:** Momentum Oscillator
*   **Concept:** The gator gauges market momentum by comparing short-term momentum to longer-term momentum, using moving averages of the midpoint price ((High + Low) / 2). The Awesome Oscillator (AO) is plotted as a histogram oscillating around zero. The gator primarily uses the zero-line crossover: Buying when the histogram crosses from negative to positive (short-term momentum overtaking long-term) and Selling when it crosses from positive to negative.
*   **Parameters:**
    *   `tg_ao_fast_length`
    ...
    *   `tg_ao_slow_length`
        ...
        *   **Constraint:** Must be greater than `tg_ao_fast_length`.
    *   *(Note: More complex AO strategies involving histogram patterns like "Saucer" or "Twin Peaks" exist but are not used for the basic TokenGator signal generation initially).*
...
# 12. Visor (Chaikin Money Flow Strategy)
